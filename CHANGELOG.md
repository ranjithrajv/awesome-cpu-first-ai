...
- **Docs**: Added [CPU-First Inference R&D Ideas](docs/cpu-first-rd-ideas.md) — documents the four core ideas behind kimi-k3-in-c (bounded resident working set, storage streaming, on-demand expert loading, expert LRU caching) with a living R&D idea tracker scoped to more ideas accelerating CPU-first inference. Added a new "R&D ideas & techniques" subsection to the README Docs section. Extended the doc with a lineage section tracing the neuron-level hot/cold split (PowerInfer, SOSP 2024) to today's expert-level disk streaming, and added tracker rows for learned shadow-model prefetch (mlx-od-moe), OS page-cache/mmap paging (midge, mlx-od-moe), io_uring/Direct I/O pumps (hummingbird, s-moe), and spec-driven model-agnostic engines (midge, hummingbird).
