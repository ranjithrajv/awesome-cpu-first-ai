...
- [llama.cpp RISC-V Support](https://github.com/ggerganov/llama.cpp) - Official support for the RISC-V Vector Extension (RVV) is integrated into `ggml`, enabling optimized LLM inference on RISC-V hardware. Dedicated optimizations for specific SoCs like SpacemiT K1/K3 are also available via specific build flags (e.g., `-DGGML_RVV=ON`).
- [SiFive Intelligence Processors](https://www.sifive.com/processors/intelligence) - Processor line specifically designed with extended vector instructions (RVV) and custom AI extensions to accelerate machine learning workloads on CPU.
- [IREE Compiler](https://iree.dev/) - Intermediate Representation Execution Environment (IREE) supports RISC-V through its LLVM-based CPU backend. It incorporates MLIR-native microkernels (ukernels) optimized for RVV to handle heavy operations like matrix multiplication efficiently.
- **Hardware Benchmarking Note:** When evaluating RISC-V LLM benchmarks, it is critical to distinguish between server-class hardware (e.g., Sophon SG2042 with 64 cores) which provides usable tokens-per-second, and consumer/SBC boards (e.g., K230, StarFive VisionFive 2). The latter typically lack powerful vector extensions or memory bandwidth, making them suitable only for very small models (e.g., TinyLlama) and proof-of-concept testing rather than performant LLM inference.
